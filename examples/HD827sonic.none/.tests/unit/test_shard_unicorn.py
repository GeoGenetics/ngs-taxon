...
def test_shard_unicorn(conda_prefix):

    with tempfile.TemporaryDirectory() as tmpdir:
        workdir = Path(tmpdir) / "workdir"
        config_path = Path(".tests/unit/shard_unicorn/config")
        data_path = Path(".tests/unit/shard_unicorn/data")
        expected_path = Path(".tests/unit/shard_unicorn/expected")
